...
void load_all_slices() {
    ...
    cacheInitCalendars((long) min_value, (long) max_value);
...
Datum imcx_info(PG_FUNCTION_ARGS) {
    ...
    add_row_to_tuple(attInMetadata, tuplestorestate,
...
void report_names(gpointer _key, gpointer _value, gpointer _user_data) {
    ...
    add_row_to_tuple(tupleData->attInMetadata, tupleData->tuplestorestate,
                     key,
                     value);
}
...
int pg_calcache_report(int showEntries, int showPageMapEntries, FunctionCallInfo fcinfo) {
    ...
    ReturnSetInfo *pSetInfo = (ReturnSetInfo *) fcinfo->resultinfo;
    Tuplestorestate *tuplestorestate;
    AttInMetadata *attInMetadata;
//    MemoryContext queryContext;
    ...
    // --
    /* check to see if caller supports us returning a tuplestorestate */
    if (pSetInfo == NULL || !IsA(pSetInfo, ReturnSetInfo))
        ereport(ERROR,
                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                        errmsg("set-valued function called in context that cannot accept a set")));
    if (!(pSetInfo->allowedModes & SFRM_Materialize))
        ereport(ERROR,
                (errcode(ERRCODE_SYNTAX_ERROR),
                        errmsg("materialize mode required, but it is not allowed in this context")));
    /* Build tuplestorestate to hold the result rows */
    oldContext = MemoryContextSwitchTo(pSetInfo->econtext->ecxt_per_query_memory); // Switch To Per Query Context
    ...
    tuplestorestate = tuplestore_begin_heap(true, false, work_mem); // Create TupleStore
    pSetInfo->returnMode = SFRM_Materialize;
    pSetInfo->setResult = tuplestorestate;
    pSetInfo->setDesc = tupleDesc;
    MemoryContextSwitchTo(oldContext); // Switch back to previous context
    attInMetadata = TupleDescGetAttInMetadata(tupleDesc);
    // --
    add_row_to_tuple(attInMetadata, tuplestorestate,
    ...
    for (uint64 i = 0; i < cacheCalendarCount; i++) {
        ...
        if (curr_calendar.calendar_id != 0) {
            ...
            getNameData.searchId = convertUIntToStr(curr_calendar.calendar_id);
            g_hash_table_foreach(cacheCalendarNameHashTable, get_name, &getNameData);
            // --
            add_row_to_tuple(attInMetadata, tuplestorestate,
                             "   Name", getNameData.name);
            add_row_to_tuple(attInMetadata, tuplestorestate,
                             "   Entries", convertIntToStr(curr_calendar.dates_size));
            add_row_to_tuple(attInMetadata, tuplestorestate,
                             "   Page Map Size", convertIntToStr(curr_calendar.page_map_size));
            add_row_to_tuple(attInMetadata, tuplestorestate,
                             "   Page Size", convertIntToStr(curr_calendar.page_size));
            if (showEntries)
                for (uint32 j = 0; j < curr_calendar.dates_size; j++) {
                    elog(INFO, "Entry[%d]: %d", j, curr_calendar.dates[j]);
                    add_row_to_tuple(attInMetadata, tuplestorestate,
                                     "   Entry",
                                     convertIntToStr(curr_calendar.dates[j]));
                }
            if (showPageMapEntries)
                for (uint32 j = 0; j < curr_calendar.page_map_size; j++) {
                    ...
                    add_row_to_tuple(attInMetadata, tuplestorestate,
                                     "   PageMap Entry",
                                     convertIntToStr(curr_calendar.page_map[j]));
                }
        } else {
            noCalendarIdCounter++;
        }
